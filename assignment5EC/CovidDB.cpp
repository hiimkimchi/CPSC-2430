...
CovidDB::CovidDB() {
    size = 17;
    hashtable = vector<vector<DataEntry*>>(size);
}
...
//desc: destroys the hashtable
//pre : table must be instantiated
//post: the entire table is deleted
void CovidDB::destroy() {
...
//desc: rehashes the table using a new size (the next prime number)
//pre : table must be instantiated
//post: -new size is found via next prime number
//      -vector is readjusted accordingly
void CovidDB::rehash() {
    //find new size
...
//pre : size must be >= 1
//post: returns false if it has factors, true if it is prime
bool CovidDB::isPrime(int size) {
    // Check from 2 to n-1
