...
ShelterBST::TreeNode* ShelterBST::parent(TreeNode* root, string name) {
    ...
    //if the root's name = name return nullptr
    if (root->pet->name == name) {
        return nullptr;
    }

    //do a search function again, but make the base case check the children
    if (root->right != nullptr && root->right->pet->name == name) {
        return root;
    }
...
ShelterBST::TreeNode* ShelterBST::predecessor(TreeNode* root, string name){
    TreeNode* petNode = search(root, name);
    if ((petNode == nullptr || petNode->left == nullptr && petNode->right == nullptr)) {
        return nullptr;
    }

    //greatest value in the left subtree
    petNode = petNode->left;
    if (petNode->right != nullptr) {
        return predecessor(petNode->right, name);
    }
    return petNode;
}
...
//desc: counts the number of nodes in a certain level
//pre : none
//post: -returns the number of nodes at level
int ShelterBST::nodesInLevel(TreeNode* root, int level) {
    ...
    //use level order traversal until it is the desired level
    ...
    while (!nodeQ.empty()) {
        ...
        for (int i = 0; i < size; i++) {
            TreeNode* frontNode = nodeQ.front();
            nodeQ.pop();
            if (currentLevel == level) {
                count++;
            }
            if (frontNode->left != nullptr) {
                nodeQ.push(frontNode->left);
            }
            if (frontNode->right != nullptr) {
                nodeQ.push(frontNode->right);
            }
        }
        currentLevel++;
    }
...
ShelterBST::TreeNode* ShelterBST::deleteNode(TreeNode* root, string name) {
    ...
    if (name < root->pet->name) {
        ...
        //one right child
            ...
            TreeNode* temp = root;
            TreeNode* parental = parent(root, name);
            root = root->right;
            if (parental != nullptr) {
                parental->right = root;
            }
        ...
        //two children
            ...
            root->pet = temp->pet;
            root->right = deleteNode(root->right,temp->pet->name);
        }
    }
    return root;
}

//desc: destroys the entire tree
//pre : none
//post: -returns nullptr when the tree is deleted
...
void ShelterBST::deletePet(string name) {
    ...
    if (petNode != nullptr) {
        cout << endl << name << " has been deleted from the tree." << endl;
    } else {
